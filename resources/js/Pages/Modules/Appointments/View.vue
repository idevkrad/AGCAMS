<template>
    <b-modal v-model="showModal" @ok="create($event)" :size="(info.status.name == 'Approved' || info.status.name == 'Completed') ? 'lg' : ''" title="View Appointment" centered v-if="info.student">
                ...
                <div class="row">
                    <div class="col-lg-12 align-self-center mt-n1">
                        ...
                        <div class="mt-0 pt-2">
                            ...
                            <span class="fw-bold font-size-13">{{ info.dte }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods: {
            show(info) {
                this.info = info.event.extendedProps;
                if (this.info.note) {
